...
const char* puzzle_find(int depth, int maxPawns, unsigned seed) {
    static std::string result;

    PuzzleGenerator::Puzzle puzzle = PuzzleGenerator::findPuzzle(depth, maxPawns, seed);
    ...
    out << "\"cards\":{";
    out << "\"red\":[" << serializeCard(puzzle.node.m_cards, Cards::CardA1) << ","
        << serializeCard(puzzle.node.m_cards, Cards::CardA2) << "],";
    out << "\"blue\":[" << serializeCard(puzzle.node.m_cards, Cards::CardB1) << ","
        << serializeCard(puzzle.node.m_cards, Cards::CardB2) << "],";
    out << "\"side\":" << serializeCard(puzzle.node.m_cards, Cards::CardExchange);
    out << "},";
    ...
    BestMove debugSearch(2 * depth, 1);
