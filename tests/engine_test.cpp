...
TEST(Engine, Puzzle1_BlueWinIn2)
{
    ...
    Node node = test::decodePosition("AAAQQgYQAAAQcMtp");
...
{
    // Load position from hash: Blue to move, wins in 3
    Node node = test::decodePosition("AAAQxAgEACAAGsIv");
...
TEST(Engine, Puzzle5_BlueWinIn4)
{
    ...
    Node node = test::decodePosition("GAAABAAACCAAeEpn");
...
{
    // Load position from hash: Blue to move, wins in 4
    Node node = test::decodePosition("AACAWAAAACABqBRA");
...
TEST(Engine, Puzzle9_BlueWinIn6)
{
    // Load position from hash: Blue to move, wins in 6
    Node node = test::decodePosition("AAAQACAsAAAAYMkF");
    ...
    int mateIn = test::matePlies(score, 11);
    EXPECT_EQ(mateIn, 11); // 6 plies from blue, 5 plies from red
}

TEST(Engine, Puzzle_StartingPositionBlueWinIn4)
{
    // Special case: this is the standard Onitama starting array (every piece on
    // its back rank, masters centred), not a contrived endgame. With the cards
    // Red[Crab,Eel] / Blue[Dragon,Frog] and Tiger in the middle, Blue to move
    // has a forced win in 4 -- a forced win straight from the opening.
    Node node = test::decodePosition("5EgAAAAAACBFIg8B");
